...
bool isModifying = false; //修改状态flag
void ModiftRoomPrice::on_pbnModify_clicked()
{
    ...
    ui->pbnSave->setEnabled(true);
    ui->pbnCancel->setEnabled(true);
    isModifying = true;//单击修改按钮，设置修改标志为正在修改
}
...
void ModiftRoomPrice::on_pbnSave_clicked()
{
    ...
    {
        //判断修改标志符，若为true则为修改，若为false则为新增
        if (isModifying = true)
        {
            QSqlQuery updateSql;
            bool status = updateSql.prepare("update roomtype set Typename ='"+roomtypeName+"',TypePrice='"+roomtypePrice+"' where RoomTypeId='"+roomtypeNo+"';");
            updateSql.exec();
            isModifying = false;//取消修改模式
            qDebug() << status;
        ...
        {
            QSqlQuery query;
            bool ok = query.prepare("INSERT INTO roomtype (RoomTypeId, Typename,TypePrice) VALUES (:Id,:Typename,:TypePrice);");
